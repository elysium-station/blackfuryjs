...
export class QueryClientImpl implements Query {
  ...
  Airdrops(request: QueryAirdropsRequest): Promise<QueryAirdropsResponse> {
    ...
    const promise = this.rpc.request("blackfury.vesting.v1.Query", "Airdrops", data);
  ...
  Airdrop(request: QueryAirdropRequest): Promise<QueryAirdropResponse> {
    ...
    const promise = this.rpc.request("blackfury.vesting.v1.Query", "Airdrop", data);
    return promise.then((data) => QueryAirdropResponse.decode(new _m0.Reader(data)));
  }

  Params(request: QueryParamsRequest): Promise<QueryParamsResponse> {
    const data = QueryParamsRequest.encode(request).finish();
    const promise = this.rpc.request("blackfury.vesting.v1.Query", "Params", data);
