...
export class Coins implements Number<Coins>, Iterable<Coin> {
  ...
  constructor(arg: Coins.Input = {}) {
    if (arg instanceof Coins) {
      ...
      if (!Array.isArray(arg)) {
        coins = [];
        Object.keys(arg).forEach((denom) => coins.push(new Coin(denom, arg[denom])));
      } else {
        coins = arg;
      }
  ...
  public add(other: Coin | Coins): Coins {
    if (other instanceof Coin) {
      ...
      return new Coins([...Object.values(other._coins), ...Object.values(this._coins)]);
    }
  }
